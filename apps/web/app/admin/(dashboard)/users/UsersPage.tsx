...
export default function UsersPage({ permissions }: { permissions: Permissions }) {
  ...
  const deleteUser = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (res.status === 401) throw new Error('You do not have permission to delete users.');
  ...
  const bulkDelete = useMutation({
    mutationFn: async (userIds: string[]) => {
      ...
      if (res.status === 401) throw new Error('You do not have permission to bulk delete users.');
  ...
  const columns = [
    ...(permissions.canEdit || permissions.canBulk || permissions.canDelete ? [columnHelper.display({
      id: 'select',
      header: () => {
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3">
          {permissions.canBulk && ( // CSV import is a bulk-level operation
            <button
      ...
      {selectedUserIds.length > 0 && (
        (permissions.canEdit || permissions.canBulk || permissions.canDelete) && (
