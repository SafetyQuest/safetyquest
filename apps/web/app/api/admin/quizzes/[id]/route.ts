// apps/web/app/api/admin/quizzes/[id]/route.ts
...
export async function GET(
  ...
  const authCheck = checkPermission(session, 'quizzes', 'view');
...
export async function PATCH(
  ...
  const authCheck = checkPermission(session, 'quizzes', 'edit');
  if (!authCheck.authorized) {
    return NextResponse.json({ error: authCheck.reason || 'Unauthorized' }, { status: 401 });
  }
...
export async function DELETE(
  ...
  const authCheck = checkPermission(session, 'quizzes', 'delete');
  if (!authCheck.authorized) {
    return NextResponse.json({ error: authCheck.reason || 'Unauthorized' }, { status: 401 });
  }
