// apps/web/components/games/shared/DragDropResultsWithFeedbackCard.tsx
'use client';
...
type DragDropResultsWithFeedbackCardProps = {
  config: {
    items: DragDropItem[];
...
type EnrichedItem = DragDropItem & {
  userTargetId: string;
...
export default function DragDropResultsWithFeedbackCard({
  config,
  userActions,
  metrics,
  mode,
  onTryAgain,
}: DragDropResultsWithFeedbackCardProps) {
  ...
  const { correctItems, incorrectItems } = useMemo(() => {
    ...
    config.items.forEach((item) => {
      ...
      const enrichedItem: EnrichedItem = {
        ...item,
        userTargetId,
        userTargetLabel: getTargetLabel(userTargetId),
    ...
    return { correctItems: correct, incorrectItems: incorrect };
                      ...
                      {correctItems.map((item, idx) => (
                        <li key={idx} className="flex items-start gap-1">
                          ...
                          <span 
                            ...
                            style={{ color: 'var(--success-dark)' }}
                          >
                            {item.content} → {item.correctTargetLabel}
                          </span>
                        </li>
                      ))}
                    </ul>
                      ...
                      {incorrectItems.map((item, idx) => (
                        <li key={idx} className="flex items-start gap-1">
                          ...
                          <span 
                            ...
                            style={{ color: 'var(--danger-dark)' }}
                          >
                            {item.content} → {item.userTargetLabel} (should be {item.correctTargetLabel})
                          </span>
                        </li>
                      ))}
                    </ul>
