...
export function BulkAssignModal({ selectedUserIds, onClose, onSuccess }: BulkAssignModalProps) {
  ...
  // Fetch all programs (LOGIC PRESERVED)
  ...
  // Process programs with assignment status (LOGIC PRESERVED)
  useEffect(() => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[var(--background)] rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-[var(--border)]">
        ...
        {isLoadingAssignments ? (
          <div className="text-center py-8"><div className="animate-pulse text-[var(--text-primary)]">Loading program assignments...</div></div>
        ) : (
          <>
                  ...
                  {filteredPrograms.map((program) => (
                    <label
                      ...
                      <div className="flex-1">
                        <div className="font-medium text-[var(--text-primary)]">{program.title}</div>
                        {program.hasInheritedAssignments && action === 'deassign' && (
                          <div className="text-xs text-[var(--warning-dark)] mt-1 flex items-start gap-1">
                            ...
                            <span>
                              {program.inheritedAssignmentsCount} user(s) have inherited assignments
                            </span>
                          </div>
                        )}
                      </div>
